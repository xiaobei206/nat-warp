...
wget -N https://cdn.jsdelivr.net/gh/YG-tsj/EUserv-warp/wireguard-go
...
echo | wgcf register
wgcf generate
sed -i "5 s/^/PostUp = ip -4 rule add from $(wget -qO- ipv4.ip.sb) table main\n/" wgcf-profile.conf
sed -i "6 s/^/PostDown = ip -4 rule delete from $(wget -qO- ipv4.ip.sb) table main\n/" wgcf-profile.conf
